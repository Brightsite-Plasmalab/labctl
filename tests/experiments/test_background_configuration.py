...
def test_make_name_list_expected_sequences(config: BackgroundConfiguration, foreground_num: int, expected: list[str]):
...
def test_make_name_list_rejects_zero_foreground_frames(config: BackgroundConfiguration):
    with pytest.raises(ValueError, match="foreground_num"):
        config.make_name_list(0)
...
def test_measurement_count_matches_generated_names(config: BackgroundConfiguration, foreground_num: int):
...
def test_measurement_count_rejects_zero_foreground_frames():
    with pytest.raises(ValueError, match="foreground_num"):
        BackgroundConfiguration.BEGIN.measurement_count(0)
